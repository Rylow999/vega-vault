#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""DSCN-G v0.14c — mismo transformer backprop manual que v0.14b PERO con mas
capacidad (D=16, corpus 30k) para confirmar si el accuracy sube con el tamano.
Hipotesis: v0.14b (D=8) no alcanzaba capacidad; con D=16 el modelo debe
concentrar la distribucion y superar v0.6a (10.11%)."""
import json, math, random, re, time
D=16; V=150; W=4; LR=0.05; SEED=0; CORPUS_N=30000; BETA=0.10
...
def main():
    print("=== v0.14c HIBRIDO backprop manual D=16 (mas capacidad) ===")
    ...
    for step in range(W,len(seq)):
        ...
        tot+=1
    acc=ok/tot
    out=dict(experiment="v0.14c_hibrido_backprop_D16",
             hypothesis="Transformer D=16 con backprop manual supera bigrama v0.6a (10.11%).",
             params=dict(d=D,V=V,window=W,lr=LR,corpus_n=CORPUS_N,aprendizaje="backprop manual"),
             acc_hibrido=round(acc,4), baseline_v06a=0.1011,
             nota="v0.14b (D=8) no alcanzaba capacidad; este sube D para confirmar.")
    with open("results_v14c.json","w") as f: json.dump(out,f,indent=2)
    print(f"acc hibrido D16={acc:.4f}  (baseline v0.6a=0.1011)")
    print("\n-> results_v14c.json")
