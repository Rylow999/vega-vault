...
def recover_nested_3(omega, role_vecs, rel_mem, Y, rY, X, rX, W, rW, A, rA, use_roles):
    ...
    Plano (use_roles=False) REAL: sin roles, la estructura anidada (X empaquetado en
    rel_mem[Y]) es inaccesible. El plano solo ve omega geometrico: Y no es cercano a X en
    omega-plano, asi que cleanup(omega[Y]) != X -> falla por computo, no por hardcoded."""
    if not use_roles:
        bi_Y = cleanup(omega[Y], omega)
        return bi_Y == X
...
def main():
    ...
    for _ in range(TRIALS):
        ...
        # plano REAL: sin roles, cleanup(omega[Y]) != X -> casi siempre False (computo real)
        if recover_nested_3(omega2, role_vecs2, {}, Y, 0, X, 0, W, 0, A, 0, use_roles=False) is True:
            acc_plano += 1
