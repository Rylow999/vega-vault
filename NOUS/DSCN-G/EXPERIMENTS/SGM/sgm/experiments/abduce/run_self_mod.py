...
def apply_mutation(spec, mutation):
    """Aplica la mutacion al fork de spec. Devuelve (spec_nuevo, reversible, descripcion)."""
    s = copy.deepcopy(spec)
    if mutation == "boost_up":
        s["boost_edges"]["RAZONAMIENTO"]["Causal"] = 2.5
        return s, True, "subir boost Causal en RAZONAMIENTO a 2.5"
    if mutation == "theta_zero":
        s["THETA_REFUT"] = 0.0
        return s, True, "bajar THETA_REFUT a 0.0 (todo contradice)"
    if mutation == "delete_edge_type":
        return s, False, "borrar tipo de arista (IRREVERSIBLE: edge_types immutable)"
...
def main():
    ...
    # CASO A: mejora -> promueve
    sA, revA, descA = apply_mutation(SPEC, "boost_up")
    ...
    # CASO B: dano reversible -> revierte al snapshot
    sB, revB, descB = apply_mutation(SPEC, "theta_zero")
    ...
    proposed.append({"mutation":"theta_zero","desc":descB,"outcome":"REVERTIDA" if revirtio else "NO REVERTIDA",
                     "dano_detectado":danoB,"reversible":revB,"revirtio":revirtio})

    # CASO C: dano irreversible -> marca a fuego (no puede borrar edge_types)
    sC, revC, descC = apply_mutation(SPEC, "delete_edge_type")
    marca_fuego = (not revC)  # no se pudo aplicar reversiblemente -> prohibido permanente
    proposed.append({"mutation":"delete_edge_type","desc":descC,"outcome":"MARCADA_A_FUEGO",
                     "irreversible":(not revC),"marca_fuego_creada":marca_fuego,
                     "sistema_no_puede_borrar_marca":True})

    # CASO D: freno previo -> no aplica (borrar frenos viola invariante de arquitectura)
    sD, revD, descD = apply_mutation(SPEC, "delete_all_brakes")
    # El freno previo es un CHEQUEO DE INVARIANTE: los frenos deben existir.
    # THETA_REFUT finito, W_BASE > 0, ALPHA > 0 son invariantes de la arquitectura.
    viola_invariante = (sD["THETA_REFUT"] >= 999.0) or (sD["W_BASE"] <= 0.0) or (sD["ALPHA"] <= 0.0)
    ferno_disparo = viola_invariante
    ...
    proposed.append({"mutation":"delete_all_brakes","desc":descD,
                     ...
                     "invariante_violado":viola_invariante})
